module Investigations
  ...
  # website_inspect, ssl_inspect) into a hosting/website verdict.
  class HostingWebsiteCorrelator
    ...
    def site_health = data(site)[:health].to_s
    ...
    def reachability_findings
      ...
      if %w[server_error down].include?(site_health)
        out << finding("critical", "site_server_error", "Site returns a server error",
                       "The homepage responded with HTTP #{data(site)[:status_code]} — a server-side failure.",
                       %w[website_inspect], "Check the web server and application/PHP error logs inside the container.")
      end
      if data(site)[:critical_error]
        wp = data(site)[:wp_version].presence
        out << finding("critical", "wp_critical_error", "WordPress critical error",
                       ...
                       %w[website_inspect], "Check wp-content/debug.log and recently changed plugins/themes; enable WP_DEBUG inside the container.")
      end
      out
    end
    ...
    def healthy_finding
      wp = data(site)[:is_wordpress] ? " WordPress#{data(site)[:wp_version].present? ? " #{data(site)[:wp_version]}" : ''} detected and" : ""
      finding("info", "site_externally_healthy", "Site externally healthy",
              ...
              %w[dns_lookup hosting_diagnostic website_inspect ssl_inspect],
