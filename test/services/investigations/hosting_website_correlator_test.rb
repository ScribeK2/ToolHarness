...
class Investigations::HostingWebsiteCorrelatorTest < ActiveSupport::TestCase
  ...
  def healthy_runs
    [
      ...
      run_for("http_inspect", data: { "redirects_to_https" => true, "missing_headers" => [], "https_response" => { "status_code" => 200, "success" => true } }),
      run_for("ssl_inspect", data: { "certificate" => { "days_until_expiry" => 90 }, "deprecated_protocols" => [] }),
      run_for("website_health", data: { "health" => "up", "status_code" => 200, "is_wordpress" => true, "wp_version" => "6.4", "critical_error" => false, "maintenance_mode" => false })
    ]
  end
  ...
  test "site unreachable -> critical site_unreachable" do
    res = Investigations::HostingWebsiteCorrelator.new(runs_with("website_health", data: { "health" => "unreachable" }, success: false)).call
    assert_includes codes(res), "site_unreachable"
    assert_equal "critical", res.verdict_status
  end

  test "server error -> critical site_server_error" do
    res = Investigations::HostingWebsiteCorrelator.new(runs_with("website_health", data: { "health" => "server_error", "status_code" => 500 })).call
  ...
  test "WordPress critical error -> critical wp_critical_error with version in message" do
    res = Investigations::HostingWebsiteCorrelator.new(runs_with("website_health", data: { "health" => "server_error", "status_code" => 500, "critical_error" => true, "is_wordpress" => true, "wp_version" => "6.4" })).call
  ...
  test "maintenance mode -> warning maintenance_mode" do
    res = Investigations::HostingWebsiteCorrelator.new(runs_with("website_health", data: { "health" => "server_error", "status_code" => 503, "maintenance_mode" => true })).call
    assert_includes codes(res), "maintenance_mode"
  end

  test "homepage 4xx -> warning site_client_error" do
    res = Investigations::HostingWebsiteCorrelator.new(runs_with("website_health", data: { "health" => "client_error", "status_code" => 403 })).call
    assert_includes codes(res), "site_client_error"
  end

  test "no HTTPS redirect -> warning no_https_redirect" do
    res = Investigations::HostingWebsiteCorrelator.new(runs_with("http_inspect", data: { "redirects_to_https" => false, "missing_headers" => [], "http_response" => { "success" => true } })).call
  ...
  test "missing security headers -> info missing_security_headers, verdict stays healthy" do
    res = Investigations::HostingWebsiteCorrelator.new(runs_with("http_inspect", data: { "redirects_to_https" => true, "missing_headers" => ["Strict-Transport-Security"], "https_response" => { "status_code" => 200, "success" => true } })).call
