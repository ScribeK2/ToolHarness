...
class Investigations::OrchestratorTest < ActiveJob::TestCase
  ...
  test "email_delivery creates four pending runs but enqueues only the two independent probes" do
    inv = nil
    ...
    runs = inv.tool_runs.to_a
    assert_equal %w[dns_lookup email_auth_check hosting_diagnostic blacklist], runs.map(&:tool_key)
    assert runs.all? { |r| r.status == "pending" }
