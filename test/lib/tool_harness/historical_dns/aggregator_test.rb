...
class ToolHarness::HistoricalDns::AggregatorTest < ActiveSupport::TestCase
  ...
  test "merges the same record across providers: union sources, widened window" do
    records = [
      rec(:a, "1.2.3.4", Date.new(2021, 1, 1), Date.new(2023, 1, 1), "whoisfreaks"),
    ...
    assert_equal %w[whoisfreaks virustotal].sort, a.first[:sources].sort
  end

  test "marks current = records sharing the latest last_seen for the type" do
    records = [
      rec(:a, "1.1.1.1", Date.new(2021, 1, 1), Date.new(2024, 3, 1), "whoisfreaks"),
      rec(:a, "2.2.2.2", Date.new(2024, 3, 1), Date.new(2026, 6, 1), "whoisfreaks")
    ]
  ...
  test "derives per-type change transitions between eras (multi-value era grouped)" do
    records = [
      rec(:ns, "ns1.ourhost.com",        Date.new(2021, 2, 1), Date.new(2024, 3, 1), "whoisfreaks"),
      rec(:ns, "dana.ns.cloudflare.com", Date.new(2024, 3, 1), Date.new(2026, 6, 1), "whoisfreaks"),
      rec(:ns, "kurt.ns.cloudflare.com", Date.new(2024, 3, 1), Date.new(2026, 6, 1), "whoisfreaks")
    ]
  ...
  test "no changes when a type has a single era" do
    records = [rec(:mx, "10 mail.example.com", Date.new(2021, 1, 1), Date.new(2026, 1, 1), "whoisfreaks")]
